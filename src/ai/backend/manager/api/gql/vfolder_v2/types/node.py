...
class VFolderGQL(PydanticNodeMixin[VFolderNode]):
    ...
    @gql_added_field(
        BackendAIGQLMeta(
            ...
            description=(
                "Usage and quota statistics fetched on demand through the storage proxy "
                "when this field is selected. This is a very slow operation: every "
                "selection makes a round-trip to the storage proxy, and the measurement "
                "cost depends on the storage backend (e.g., a full directory walk on vfs). "
                "Select only when needed. Null for unmanaged vfolders."
            ),
        )
    )  # type: ignore[misc]
    async def usage(
        ...
        result = await info.context.adapters.vfolder.get_live_usage(UUID(self.id))
        if result is None:
            return None
